...
async def default_cooldown(
    ...
    if (
        await is_premium(interaction.user.id) != "None"
        or await is_premium_server(interaction.guild)
        or ((await db.get_user(interaction.user.id)) or {"next_bump": 0})["next_bump"]
        > time.time()
    ):
        return None
