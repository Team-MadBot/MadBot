...
class UpdateStatsCog(commands.Cog):
    ...
    async def update_stats(self):
        async for guild in db.get_guilds_stats():
            if guild["next_update"] > time.time():
                continue
            for channel_id in guild["channels"]:
                with suppress(Exception):
                    ...
                    try:
                        await channel.edit(
                            name=channel_id["text"].replace("%count%", str(stat))
                        )
                    except Exception as e:
                        if not isinstance(e, discord.Forbidden):
                            logger.error(e)
            await db.update_guild_stats(
                guild_id=guild["id"], next_update=int(time.time()) + 600
            )
    ...
    def get_stat(self, channel_type: str, guild: discord.Guild):
        ...
        match channel_type:
            ...
            case "people":
                return guild.member_count - len(
                    list(filter(lambda x: x.bot, guild.members))
                )
            case "bots":
