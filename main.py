...
async def botinfo(interaction: discord.Interaction):
    ...
    embed.add_field(name="Версия:", value=curr_version, inline=False)
    embed.add_field(name="Версия discord.py", value=f"{discord.version_info.major}.{discord.version_info.minor}.{discord.version_info.micro} `{discord.version_info.releaselevel.upper()}`", inline=False)
    embed.add_field(name="Версия Python", value=f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}", inline=False)
    embed.add_field(name="Пинг:", value=f"{int(round(bot.latency, 3)*1000)}ms", inline=False)
    embed.add_field(name="Запущен:", value=f"<t:{started_at}:R>")
    embed.add_field(name="Кол-во серверов:", value=len(bot.guilds), inline=False)
