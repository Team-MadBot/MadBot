...
class Base64(commands.GroupCog, group_name="base64"):
    ...
    async def encode(
        ...
        if len(text) > 1024 or len(ans) > 1024:
            ...
            embed1 = discord.Embed(
                title="Полученный текст:", color=discord.Color.orange(), description=ans
            )
            return await interaction.response.send_message(
                embeds=[embed, embed1], ephemeral=True
            )
        embed = (
            discord.Embed(title="Зашифровка:", color=discord.Color.orange())
            .add_field(name="Исходный текст:", value=text, inline=False)
            .add_field(name="Полученный текст:", value=ans)
        )
    ...
    async def decode(self, interaction: discord.Interaction, text: str):
        ...
        embed = (
            discord.Embed(title="Расшифровка:", color=discord.Color.orange())
            .add_field(name="Исходный текст:", value=text, inline=False)
            .add_field(name="Полученный текст:", value=ans)
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
