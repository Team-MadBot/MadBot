        ...
        class Premium(app_commands.Group):
            ...
            async def give(self, interaction: discord.Interaction):
                ...
                if await is_premium(interaction.user.id) == 'server' and premiums == 2:
                    ...
                    return await interaction.response.send_message(embed=embed, ephemeral=True)
                await coll.insert_one({'guild_id': str(interaction.guild.id), 'user_id': str(interaction.user.id)})
                embed = discord.Embed(
                    title="Успешно!",
            ...
            async def info(self, interaction: discord.Interaction):
                ...
                options = []
                async for count, prem in aenumerate(db.get_premium_guilds(interaction.user.id), start=1):
                    ...
                    options.append(
                        discord.SelectOption(
                            label=f"Снять подписку с сервера №{count}",
