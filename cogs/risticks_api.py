...
class RisticksAPI(commands.Cog):
    ...
    def bump_success(self):
        ...
        ).set_thumbnail(url=self.risticks_logo_url)

    def __init__(self, bot: commands.AutoShardedBot):
        self.bot = bot
        self._token = settings["risticks_token"]
        self.session = aiohttp.ClientSession(base_url="https://api.risticks.xyz")
    ...
    async def _post_guildinfo(
        ...
        async with self.session.post(
            f"/guildinfo/{guild_id}", headers=headers, json=json
        ) as response:
    ...
    async def _get_guildinfo(self, guild_id: int) -> dict:
        ...
        async with self.session.get(
            f"/guildinfo/{guild_id}", headers=headers
        ) as response:
            return await response.json()
    ...
    async def _bump(self, guild_id: int, user_id: int) -> dict:
        ...
        async with self.session.post(
            f"/bump/{guild_id}", json=json, headers=headers
        ) as response:
            return await response.json()
    ...
    async def bump(self, interaction: discord.Interaction):
        ...
        try:
            ...
            return await interaction.followup.send(embed=self.unknown_risticks_error)

        if resp.get("code") == 404:
            return await interaction.followup.send(embed=self.no_guild_error)
