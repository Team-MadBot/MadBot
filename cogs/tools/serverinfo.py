...
class ServerInfo(commands.Cog):
    ...
    @app_commands.command(
        name="serverinfo", description="[Полезности] Информация о сервере"
    )
    ...
    async def serverinfo(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            name="Владелец", value=f"<@!{interaction.guild.owner_id}>", inline=True
        ).add_field(
            ...
            name="Категорий", value=len(interaction.guild.categories), inline=True
        ).add_field(
            ...
            name="Трибун", value=len(interaction.guild.stage_channels), inline=True
        ).add_field(
            name="Кол-во эмодзи",
        ...
        embed.add_field(
            name="Уровень проверки", value=verification_level, inline=True
        ).add_field(
            name="Дата создания",
            ...
            value=(
                ...
                else "Недоступно (сервер не является сервером сообщества)"
            ),
        ).add_field(
            name="Веток", value=f"{len(interaction.guild.threads)}"
        )
        guild_roles = sorted(
            list(
                filter(
        ...
        embed.add_field(
            ...
            + (
                ...
                else f" и ещё {guild_roles_amount - 15} ролей..."
            ),
        )
        ...
        embed.set_footer(text=f"ID: {interaction.guild.id}").set_author(
        ...
        if interaction.guild.banner is not None:
            banner = discord.Embed(
                color=discord.Color.orange(),
                description=f"[Скачать]({interaction.guild.banner.url})",
            )
            ...
            banner = discord.Embed(
                title="Ошибка!",
                color=discord.Color.red(),
                description="У сервера отсутствует баннер!",
            )

        if interaction.guild.icon is not None:
            icon = discord.Embed(
                color=discord.Color.orange(),
                description=f"[Скачать]({interaction.guild.icon.url})",
            )
            ...
            icon = discord.Embed(
                title="Ошибка!",
                color=discord.Color.red(),
                description="У сервера отсутствует аватар!",
            )
            ...
            def __init__(self):
                options = [
                    discord.SelectOption(
                        label="Аватар",
                        value="avatar",
                        description="Получить аватар сервера.",
                        emoji="🖼️",
                    ),
                    discord.SelectOption(
                        label="Баннер",
                        value="banner",
                        description="Получить баннер сервера (при наличии).",
                        emoji="🏙️",
                    ),
                    discord.SelectOption(
                        label="Информация",
