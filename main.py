...
class MadBot(commands.AutoShardedBot):
        ...
        for path, _, files in os.walk("cogs"):
            for file in files:
                ...
                if egg in config.cogs_ignore:
                    logger.debug(
                        f"Модуль {egg} находится в игнор-листе в config.py, поэтому он не был загружен."
                    )
                    continue
                try:
                    ...
                    logger.debug(
                        f"Модуль {egg} не имеет точки входа, поэтому он не был загружен."
                    )
    ...
    async def setup_hook(self):
        ...
        if self.migrate_db:
            logger.info(
                "При запуске была указана необходимость миграции. Бот выполнит её перед полным запуском."
            )
            try:
                await self.db_migration()
            except Exception:
                logger.error(
                    "Во время миграции произошла ошибка: " + traceback.format_exc()
                )
                logger.warning(
                    "Миграция не была проведена успешно! Некоторый функционал бота может работать некорректно!"
                )
            else:
    ...
    async def on_ready(self):
        ...
        for guild in self.guilds:
            ...
            if await checks.is_in_blacklist(guild.id) or await checks.is_in_blacklist(
                guild.owner_id
            ):
                await guild.leave()
    ...
    async def is_owner(self, user: discord.abc.User) -> bool:
        return (
            False
            if await checks.is_in_blacklist(user.id)
            else True if user.id in config.coders else await super().is_owner(user)
        )
